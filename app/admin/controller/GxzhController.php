 ...
 * )
 */
class GxzhController extends AdminBaseController
{
    ...
    {
        $list = Db::name('gxzh_msg')->where(['type'=>1])->order(["id" => "DESC"])->select();
    ...
    public function notice_add()
    {
        if ($this->request->isPost()) {
            ...
            $result = Db::name('gxzh_msg')->insert($data);
            if ($result) {
                $this->success("添加成功", url("gxzh/notice"));
            } else {
                $this->error("添加失败");
            }
        }
    ...
    public function notice_edit()
    {
        ...
        if ($this->request->isPost()) {
            $data['content'] = htmlspecialchars_decode($data['content']);
            if (Db::name('gxzh_msg')->update($data) !== false) {
                $this->success("保存成功！", url('gxzh/notice'));
            } else {
                $this->error("保存失败！");
            }
        }
        $data = Db::name('gxzh_msg')->where("id", $data['id'])->find();
    ...
    public function notice_delete()
    {
        ...
        $status = Db::name('gxzh_msg')->delete($id);
        if (!empty($status)) {
            $this->success("删除成功！", url('gxzh/notice'));
        } else {
            $this->error("删除失败！");
        }
    }
